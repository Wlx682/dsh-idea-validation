...
每次 `idea_validation` 调用只推进一个阶段，返回 `caseId`、`revision`、完整状态快照和当前唯一门禁。非终态门禁通过 `ask_user_question` 显示为选择卡片；澄清阶段严格遵循“一事一卡”，三个待确认事项就是三张分别带答案选项的卡片，不会把 A/B/C 塞进一个长问题。用户完成整组卡片后，根 Agent 会把全部真实答案聚合并映射到同一个 case 的一次下一阶段调用。用户未确认时不会自动进入下一阶段；格式错误只修复当前阶段，不会重跑已经完成的工作。
...
澄清卡片的数据结构是 `clarificationQuestions[]`：每项包含稳定 id、短标题、单一问题、2–4 个可直接回答该问题的选项和单/多选标记。`openQuestions` 仅保留为旧状态兼容字段；旧 case 会逐题迁移，不再合并成一张大卡。
...
专项回归固定验证：阶段级修复、单 Agent 阶段预算、重复启动幂等、卡片工具装配、旧状态迁移、无默认工作区广扫。完整测试另外覆盖跨轮状态、revision 乐观锁、一事一卡与批量答案协议、证据授权、实验、实施交接和终态。
