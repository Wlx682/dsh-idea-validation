...
```text
六维脑补 → 轻量确认 → 问题确认 → 取证计划 → 证据结论 → 方案比较 → 最小实验 → 实施交接
```

每次 `idea_validation` 调用只推进一个阶段，返回 `caseId`、`revision`、完整状态快照和当前唯一门禁。新想法不再先做填表式追问：LLM 会直接生成核心目标、目标用户、执行路径、关键风险、所需资源和预期交付六维草案，并明确标记为“待确认推测，不是事实或证据”。界面先显示一张总览卡；用户可一键全盘通过，或选择“逐项检查”再展开六张独立卡，只删除、替换偏差项。旧 case 的 1–3 张澄清卡仍可继续使用。

## 它解决什么问题

- 用户补充的上下文成为正式状态，不再只把最初一句话反复交给新 Agent。
- 用户只需辨认偏差，不必从空白文本框创造答案；未修改维度默认接受。
- 六维脑补只能作为可证伪假设，禁止进入 evidence 或伪装成组织事实、真实预算和真实基线。
...
| 阶段 | 产物 | 人类动作 |
| --- | --- | --- |
| `clarify` | 六维待确认推测；旧 case 兼容 1–3 张澄清卡 | 全盘通过 / 逐项检查 / 整体重做 / 暂缓 / 放弃 |
...
新流程的数据结构是 `ideaExpansion`，包含使命与指标、三个用户画像、一条主路线和两条备选路线、三个致命风险、资源估算、交付物及显式假设声明。`expansion-review` 协议先交付总览；只有用户选择“逐项检查”时才交付六张一维一卡的详情卡，自定义输入代表替换该维推测。

旧流程的 `clarificationQuestions[]` 和 `clarification-batch` 协议仍保留：旧 case 会把 `openQuestions` 逐题迁移为独立卡片，不会把 A/B/C 塞进问题正文。
...
专项回归固定验证：阶段级修复、单 Agent 阶段预算、重复启动幂等、卡片工具装配、旧状态迁移、六维脑补协议和无默认工作区广扫。完整测试另外覆盖跨轮状态、revision 乐观锁、总览与六张详情卡、旧版一事一卡兼容、证据授权、实验、实施交接和终态。
